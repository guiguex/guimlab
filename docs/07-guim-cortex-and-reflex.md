...
1. **Cortex (`guim_lovelace_fused_kernel`)** — full meta-cognitive substrate, ~3-5 µs/frame, runs the bulk of learning. Uses PTX `vabsdiff4` for surprise, `__ballot_sync` for warp routing, WMMA `mma_sync` for one-shot neuromodulation, CF-CTT for tick-rate-independent traces, DDWR for 95% VRAM saved.
...
Even at 3-5 µs per frame, the cortex can't react fast enough for emergencies:
...
The cortex's 3-5 µs is too slow. We need a **separate, faster path**.
...
**vs Cortex**: 3-5 µs (~50x slower). Reflex is the priority path.
...
|---|---|---|---|---|
| Dense cortex | ~100 µs | 98K reads | 80W | Never (superseded) |
| Sparse cortex (DDWR) | 3-5 µs | ~3K reads (active only) | 5-10W | Default cognitive path |
